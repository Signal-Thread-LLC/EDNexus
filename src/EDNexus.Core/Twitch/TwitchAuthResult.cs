...
{
    /// <summary>Login completed and the EBS-issued token was persisted.</summary>
    Success,

    /// <summary>The caller's <c>CancellationToken</c> was cancelled before the flow completed.</summary>
    Cancelled,

    /// <summary>The commander declined the authorization request on Twitch's (EBS-hosted) consent page.</summary>
    Denied,
...
public sealed record TwitchAuthResult(TwitchAuthStatus Status, string? Username = null, string? ChannelId = null, string? Error = null)
{
    public bool IsSuccess => Status == TwitchAuthStatus.Success;

    public static TwitchAuthResult Ok(string username, string channelId) => new(TwitchAuthStatus.Success, username, channelId);
